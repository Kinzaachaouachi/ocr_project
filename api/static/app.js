// Vérifier le token uniquement si on est sur la page d'application (/app)
// Cela évite de rediriger automatiquement depuis la page de login
...
(function displayUser() {
  try {
    const storedUser =
      localStorage.getItem("user") || localStorage.getItem("user_info");
...
function detectLanguageFromTextClient(text) {
  ...
  if (/[\u0600-\u06FF]/.test(sample))
    return { code: "ar", name: "العربية", confidence: 0.9 };
  if (/[\u0590-\u05FF]/.test(sample))
    return { code: "he", name: "עברית", confidence: 0.9 };
  if (/[\u0400-\u04FF]/.test(sample))
    return { code: "ru", name: "Русский", confidence: 0.85 };
  if (/[\u4E00-\u9FFF]/.test(sample))
    return { code: "zh", name: "中文", confidence: 0.9 };
  if (/[\u3040-\u309F\u30A0-\u30FF]/.test(sample))
    return { code: "ja", name: "日本語", confidence: 0.85 };
  if (/[\uAC00-\uD7AF]/.test(sample))
    return { code: "ko", name: "한국어", confidence: 0.9 };
  if (/[\u0E00-\u0E7F]/.test(sample))
    return { code: "th", name: "ไทย", confidence: 0.9 };
...
function resolveDetectedLanguage(data) {
  if (data?.detected_language?.code) {
    return {
      ...
      name:
        data.detected_language.name ||
        getLanguageName(data.detected_language.code),
      confidence: data.detected_language.confidence || 0.5,
    };
  }

  const firstTextResult = (data?.results || []).find(
...
function showParallelExtractionHint() {
  loadingOverlay.innerHTML =
    '<div class="loading-message">Extraction en cours… Les 4 modèles sont lancés simultanément.</div>';
}

const originalPerformExtraction = performExtraction;
performExtraction = async function (file) {
  showParallelExtractionHint();
  await originalPerformExtraction(file);
};
...
function colorizeText(text, wordConfidence) {
  ...
  return words
    .map((word) => {
      ...
      if (confidence < 0.6) colorClass = "word-low";
    ...
    mutations.forEach(function (mutation) {
      if (mutation.type === "characterData" || mutation.type === "childList") {
        ...
        if (
          span &&
          span.classList &&
          (span.classList.contains("word-medium") ||
            span.classList.contains("word-low"))
        ) {
  ...
  document.querySelectorAll(".translate-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      const targetLang = document.getElementById(
        `translate-select-${index}`,
      ).value;
      const translatePanel = document.getElementById(
        `translation-panel-${index}`,
      );
      ...
      try {
        ...
        translationText.textContent =
          data.translated_text || "Aucune traduction disponible.";
      } catch (err) {
