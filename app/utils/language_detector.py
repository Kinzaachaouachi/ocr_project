...
def detect_language_by_charset(text: str) -> Tuple[str, float]:
    ...
    if (
        char_counts["japanese_hira"] + char_counts["japanese_kata"]
    ) > total_chars * 0.2:
        return ("ja", 0.9)
...
def detect_latin_language(text: str) -> Tuple[str, float]:
    ...
    if scores:
        ...
        confidence = (
            min(0.95, (max_score / max(total_matches, 1)) * 0.9)
            if total_matches > 0
            else 0.5
        )
...
def configure_model_for_language(model_id: str, language_code: str) -> dict:
    ...
    configs = {
        "paddleocr": {"lang": language_code, "use_angle_cls": True, "show_log": False},
        "easyocr": {"languages": [language_code, "en"], "gpu": False},
        "docling": {},
        "trocr": {},
    }
