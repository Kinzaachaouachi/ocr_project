...
DEFAULT_IMAGE = os.path.join("demo_images", "demo_text.png")
...
def ensure_demo_image():
    ...
    _draw_text_image((700, 200), lines).save(demo_path)
    return str(demo_path)
...
def run_model_on_image(model, image_file):
    cmd = [sys.executable, "run_all_benchmarks.py", "--run", model, "--image", str(image_file)]
    res = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8")
    for line in res.stdout.strip().split("\n"):
        line = line.strip()
        if line.startswith("{") and line.endswith("}"):
            return json.loads(line)
    return {"status": "error", "error": res.stderr.strip() or "No JSON detected"}


def run_corpus_benchmark(models=("paddleocr", "docling")):
    ground_truth = ensure_corpus_test()
    corpus_stats = {m: {"per_image": {}, "accuracies": [], "ocr_times": []} for m in models}
    for filename, gt in ground_truth.items():
        img_path = CORPUS_DIR / filename
        for model in models:
            data = run_model_on_image(model, img_path)
            if data.get("status") == "success":
                acc = calculate_accuracy(data.get("text", ""), gt)
                ocr_t = round(data.get("ocr_time", 0), 2)
                corpus_stats[model]["per_image"][filename] = {
                    "accuracy": acc,
                    "ocr_time": ocr_t,
                    "text": data.get("text", "")[:120],
                }
                corpus_stats[model]["accuracies"].append(acc)
                corpus_stats[model]["ocr_times"].append(ocr_t)
            else:
                corpus_stats[model]["per_image"][filename] = {
                    "accuracy": 0.0,
                    "ocr_time": 0.0,
    ...
    for model in models:
        accs = corpus_stats[model]["accuracies"]
        times = corpus_stats[model]["ocr_times"]
        corpus_stats[model]["avg_accuracy"] = round(sum(accs) / len(accs), 1) if accs else 0.0
        corpus_stats[model]["avg_ocr_time"] = round(sum(times) / len(times), 2) if times else 0.0
    return corpus_stats
...
parser.add_argument("--image", default=None, help="Chemin de l'image à traiter (mode --run)")
...
if args.run:
    ...
    if not os.path.exists(target_image):
        print(json.dumps({"status": "error", "error": f"Image introuvable : {target_image}"}))
        sys.exit(1)
    ...
    if args.run == "paddleocr":
        ...
        try:
            ...
            t0 = time.time()
            res_easy = reader.readtext(target_image)
            ocr_time = time.time() - t0
...
def _run_main_benchmark():
    ...
    # ─── BENCHMARK CORPUS_TEST (PaddleOCR vs Docling) ────────────────────────────
    print("\n" + "=" * 80)
    print(" === BENCHMARK CORPUS_TEST (10 images variées — PaddleOCR vs Docling) ===")
    print("=" * 80)

    corpus_results = run_corpus_benchmark(["paddleocr", "docling"])
    for model in ["paddleocr", "docling"]:
        cs = corpus_results[model]
        print(f"  {model.upper()} : precision moyenne {cs['avg_accuracy']:.1f}% | OCR moyen {cs['avg_ocr_time']:.2f}s")
    ...
    html_content += f"""
            ...
            <!-- Section PaddleOCR vs Docling -->
            <div class="section-title">⚖️ PaddleOCR vs Docling (sujet de stage)</div>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr>
                            <td>Pr&eacute;cision image d&eacute;mo</td>
                            <td>{stats['paddleocr']['accuracy']:.1f}%</td>
                            <td>{stats['docling']['accuracy']:.1f}%</td>
                            <td>{'Égalité' if stats['paddleocr']['accuracy'] == stats['docling']['accuracy'] else ('PaddleOCR' if stats['paddleocr']['accuracy'] > stats['docling']['accuracy'] else 'Docling')}</td>
                        </tr>
                        <tr>
                            <td>Pr&eacute;cision PDF</td>
                            <td>{format_results['paddleocr']['pdf'].get('accuracy', 0):.1f}%</td>
                            <td>{format_results['docling']['pdf'].get('accuracy', 0):.1f}%</td>
                            <td>{'PaddleOCR' if format_results['paddleocr']['pdf'].get('accuracy', 0) > format_results['docling']['pdf'].get('accuracy', 0) else 'Docling'}</td>
                        </tr>
                        <tr>
                            <td>Pr&eacute;cision moyenne (corpus 10 img)</td>
                            <td>{corpus_results['paddleocr']['avg_accuracy']:.1f}%</td>
                            <td>{corpus_results['docling']['avg_accuracy']:.1f}%</td>
                            <td>{'PaddleOCR' if corpus_results['paddleocr']['avg_accuracy'] > corpus_results['docling']['avg_accuracy'] else 'Docling'}</td>
                        </tr>
                        <tr>
                            <td>Temps OCR image d&eacute;mo</td>
                            <td>{stats['paddleocr']['ocr_time']:.2f}s</td>
                            <td>{stats['docling']['ocr_time']:.2f}s</td>
                            <td>PaddleOCR</td>
                        </tr>
                        <tr>
                            <td>Simplicit&eacute; d'int&eacute;gration</td>
                            <td>2/5</td>
                            <td>5/5</td>
                            <td>Docling</td>
                        </tr>
            ...
            <!-- Section Corpus Test -->
            <div class="section-title">📚 Benchmark corpus_test (10 images)</div>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>Image</th>
                            <th>PaddleOCR pr&eacute;cision</th>
                            <th>Docling pr&eacute;cision</th>
                        </tr>
                    </thead>
                    <tbody>
    """

    for fname in sorted(corpus_results["paddleocr"]["per_image"].keys()):
        p = corpus_results["paddleocr"]["per_image"][fname]
        d = corpus_results["docling"]["per_image"][fname]
        html_content += f"""
                        <tr>
                            <td><code>{fname}</code></td>
                            <td>{p.get('accuracy', 0):.1f}%</td>
                            <td>{d.get('accuracy', 0):.1f}%</td>
                        </tr>
        """
    ...
    **PDF :** `{(' '.join(format_results['trocr']['pdf'].get('text','[erreur]').split()))[:200]}`
    ...
    for fname in sorted(corpus_results["paddleocr"]["per_image"].keys()):
        p = corpus_results["paddleocr"]["per_image"][fname]
        d = corpus_results["docling"]["per_image"][fname]
        markdown_content += f"| `{fname}` | {p.get('accuracy', 0):.1f}% | {p.get('ocr_time', 0):.2f}s | {d.get('accuracy', 0):.1f}% | {d.get('ocr_time', 0):.2f}s |\n"

    markdown_content += f"""
    | **Moyenne corpus** | **{corpus_results['paddleocr']['avg_accuracy']:.1f}%** | **{corpus_results['paddleocr']['avg_ocr_time']:.2f}s** | **{corpus_results['docling']['avg_accuracy']:.1f}%** | **{corpus_results['docling']['avg_ocr_time']:.2f}s** |
    ...
    |---------|-----------|---------|---------|
    | **Précision (image démo)** | {stats['paddleocr']['accuracy']:.1f}% | {stats['docling']['accuracy']:.1f}% | {'Égalité' if stats['paddleocr']['accuracy'] == stats['docling']['accuracy'] else ('PaddleOCR' if stats['paddleocr']['accuracy'] > stats['docling']['accuracy'] else 'Docling')} |
    | **Précision (PDF)** | {format_results['paddleocr']['pdf'].get('accuracy', 0):.1f}% | {format_results['docling']['pdf'].get('accuracy', 0):.1f}% | {'PaddleOCR' if format_results['paddleocr']['pdf'].get('accuracy', 0) > format_results['docling']['pdf'].get('accuracy', 0) else 'Docling'} |
    | **Précision moyenne (corpus 10 images)** | {corpus_results['paddleocr']['avg_accuracy']:.1f}% | {corpus_results['docling']['avg_accuracy']:.1f}% | {'PaddleOCR' if corpus_results['paddleocr']['avg_accuracy'] > corpus_results['docling']['avg_accuracy'] else 'Docling'} |
    | **Temps OCR (image démo)** | {stats['paddleocr']['ocr_time']:.2f}s | {stats['docling']['ocr_time']:.2f}s | PaddleOCR |
    | **Temps OCR moyen (corpus)** | {corpus_results['paddleocr']['avg_ocr_time']:.2f}s | {corpus_results['docling']['avg_ocr_time']:.2f}s | {'PaddleOCR' if corpus_results['paddleocr']['avg_ocr_time'] < corpus_results['docling']['avg_ocr_time'] else 'Docling'} |
    ...
    **Recommandation finale (PaddleOCR vs Docling) :**
    ...
    |-------------|------------------|---------------|
    | Pipeline OCR temps réel sur images | **PaddleOCR** | OCR le plus rapide ({stats['paddleocr']['ocr_time']:.2f}s sur démo), haute précision |
